...
SSM_DOC_NAME = PREFIX + 'attach-iam-instance-already-assoc'
CFN_STACK_NAME = PREFIX + 'attach-iam-instance-already-assoc'
TEST_CFN_STACK_NAME = PREFIX + 'attach-iam-instance-already-assoc'
    ...
    def test_update_document(self):
        ...
        test_cf_stack = ssm_testing.CFNTester(
            ...
            template_filename=os.path.abspath(os.path.join(
                DOC_DIR,
                "Tests/CloudFormationTemplates/TestAssociated.yml")),
        ...
        try:
            ...
            test_role_name = test_cf_stack.stack_outputs['TestRoleName']
            ...
            self.assertEqual(test_role_name, payload['RoleName'])
            ...
            for role in instance_profile_response['InstanceProfile']['Roles']:
                if role['RoleName'] == test_role_name:
                    role_count += 1

            self.assertEqual(role_count, 1)

            LOGGER.info('Tests successful. Cleaning up')
            remove_role_from_instance_profile(profile_name, test_role_name)
